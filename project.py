...
class Project():
    ...
    def step(self, time_step):
        ...
        if time_step % (240 // 40) == 0: # kalman filter only runs 40hz
        ...
        if self.state == "restart":
            if (
                ...
                and self.robot.check_if_ee_is_stoped()
            ):
                self.state = "start"
            else:
            ...
            while True:
                ...
                if grasp is None:
                    ...
                    self.gripper_t = grasp[0] - np.array([0, 0, 0.035])
            ...
            preposition = self.gripper_t + np.asarray(p.getMatrixFromQuaternion(self.r_object)).reshape(3, 3) @ np.array([0, 0, -0.1])
            if self.robot.check_if_ee_reached(preposition) and self.robot.check_if_ee_reached_orientation(self.r_object):
            ...
            if self.robot.check_if_ee_reached(self.start_position):
                ...
                self.vis.plot_trajectory([], [], self.obstacle_tracking.get_obstacles(), self.start_position, currentEE)
                self.robot.control(self.start_position, DOWN_GRIPPER, 1)
            ...
            if self.robot.check_if_ee_reached(self.target_position):
                ...
                if not sucess:
                    ...
                    self.vis.plot_trajectory(trajectory, trajectory_support_points, self.obstacle_tracking.get_obstacles(), current_target, currentEE)
            ...
            self.gripper_state = "open"
            print("check if gripper is open", self.robot.check_if_gripper_open(), not self.robot.check_if_gripper_closed())
            ...
            if self.robot.check_if_ee_reached(self.start_position) and self.robot.check_if_ee_is_stoped():
