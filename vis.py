...
class Vis:
    ...
    def plot_kf_error(self, time_step, obstacle_tracking):
        ...
        self.axs.set_ylabel("Tracking error (cm)")

        self.time_arr.append(time_step/240)
        ...
        j = 0
        for i, key in enumerate(obstacle_tracking.kf):
            ...
            self.plot_arr[4*i].append(100*error)
            self.plot_arr[4*i+1].append(np.abs(kf-gt)[0])
            self.plot_arr[4*i+2].append(np.abs(kf-gt)[1])
            self.plot_arr[4*i+3].append(np.abs(kf-gt)[2])

            #self.axs[j].plot(self.time_arr, self.plot_arr[4*j], label=key+"_obstacle")
            self.axs.plot(self.time_arr, self.plot_arr[4*i], label=key+"_obstacle")
